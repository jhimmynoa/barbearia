  ...
  return (
    <div className="min-h-screen bg-[#050505]">
      <style>{`
        ...
        .animate-zoom.animate-on-scroll {
      ...
      {/* 
        HERO SECTION: 
        Changed to 'sticky top-0 h-screen' to stay fixed while content scrolls over it.
        Added 'z-0' to ensure it sits behind the content.
      */}
      <header className="sticky top-0 h-screen flex flex-col justify-between overflow-hidden z-0">
        ...
        {/* Navbar */}
        <nav className="absolute top-0 w-full z-50 px-6 py-6 flex justify-between items-center border-b border-white/5 backdrop-blur-sm animate-on-scroll is-visible">
        ...
        <div className="flex-grow flex flex-col items-center justify-center text-center relative z-10 px-4 mt-20">
          ...
          <p className="animate-on-scroll delay-100 text-white text-[10px] md:text-xs tracking-[0.3em] font-bold mb-2">EXCLUSIVO PARA HOMENS</p>
          <h1 className="animate-on-scroll delay-200 text-5xl sm:text-6xl md:text-9xl font-display font-bold text-transparent bg-clip-text bg-gradient-to-br from-orange-400 via-orange-500 to-red-500 mb-2 tracking-tighter drop-shadow-[0_0_35px_rgba(249,115,22,0.6)]">BERGER</h1>
          <div className="animate-on-scroll delay-300 flex flex-col md:flex-row items-center justify-center gap-2 md:gap-4 text-xs md:text-sm tracking-[0.2em] text-gray-400 mb-6">
          ...
          <p className="animate-on-scroll delay-400 text-gray-300 italic max-w-md mx-auto mb-10 text-sm md:text-base font-light px-4">
            Cortes precisos, barba alinhada e um ambiente feito para quem exige mais do que o básico.
          </p>
          <button 
            onClick={() => navigate('/calendar')}
            className="animate-on-scroll delay-500 bg-gradient-to-r from-orange-500 to-orange-600 hover:to-orange-500 text-white text-xs font-bold py-4 px-8 tracking-widest uppercase transition-all duration-300 shadow-[0_0_20px_rgba(249,115,22,0.4)] hover:shadow-[0_0_40px_rgba(249,115,22,0.6)] w-full md:w-auto max-w-xs rounded md:rounded-none">
            Agendar Horário
          </button>
        </div>
        
        {/* Scroll Indicator (Hidden on Mobile) */}
        <div className="animate-on-scroll delay-500 absolute left-6 top-1/2 -translate-y-1/2 hidden lg:flex flex-col items-center gap-4 text-[10px] text-orange-500 font-bold tracking-widest [writing-mode:vertical-rl] rotate-180 drop-shadow-[0_0_5px_rgba(249,115,22,0.8)]">
      ...
      {/* 
        CONTENT WRAPPER:
        Added 'relative z-10 bg-[#050505]' to create a layer that slides OVER the sticky hero.
        The background color is crucial so it's opaque.
      */}
      <div className="relative z-10 bg-[#050505]">
        ...
        <section className="py-20 relative border-t border-white/5" style={gridStyle}>
          <div className="container mx-auto px-4 md:px-8 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                { icon: 'fa-scissors', title: 'Corte Premium', desc: 'Corte personalizado com técnicas modernas e acabamento impecável.', price: 'R$ 60' },
                { icon: 'fa-wand-magic-sparkles', title: 'Barba Completa', desc: 'Design, aparo e finalização com produtos premium.', price: 'R$ 45' },
                { icon: 'fa-pen-nib', title: 'Combo Especial', desc: 'Corte + Barba com tratamento completo e hidratação.', price: 'R$ 95' },
                { icon: 'fa-fire', title: 'Degradê Premium', desc: 'Degradê high fade ou low fade com transições perfeitas.', price: 'R$ 70' }
              ].map((service, index) => (
                <div key={index} className={`animate-on-scroll delay-${index * 100} group bg-card-dark border border-card-border p-8 hover:border-orange-500 transition-colors duration-300 rounded-sm hover:shadow-[0_0_20px_rgba(249,115,22,0.1)]`}>
                  ...
                  <h3 className="text-white font-display font-bold text-xl mb-3">{service.title}</h3>
                  <p className="text-gray-500 text-xs leading-relaxed mb-6">{service.desc}</p>
                  <p className="text-orange-500 font-bold text-2xl">{service.price}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 relative" style={gridStyle}>
          <div className="container mx-auto px-4 md:px-8 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                "https://lh3.googleusercontent.com/aida-public/AB6AXuCgN8QjovH4X0j_hLv9_Re3638KFbbPzBqiGRQGZ4KJMOlrcspXAWVftJ7dfRDNpx-qj78WPRqyzGn3O3zEJp0v5a9gw-vOxIklmNZfQi0RDLBK3QYqLFg7fIERf8xf5grM_D51ubwzO0D8cgAmITZ4JjSJGF22ZqvqiMkVreo4oeqpHiEbuRtCnKwKxTuVx0MT8pmJrHYzddiBrs7V3SEAwlIAmeH076oEpyRobOgZfGdUK_NjuIQXrQXSnWHOTLLOiHcrerhooZ69",
                "https://lh3.googleusercontent.com/aida-public/AB6AXuA1EUCgGcjNSBoJdfbmYPzhoySt1-5jtjOzqMJuTkXHvTTeA4ahdW1jGwJ0DsyKWrt7KY-_qlrKneh3egmKt355xY4sICV3P4UDmROSYtrRDr5OMzuZWzemqXJM7UH5OMRluiH0Nq6e1TJbEXY1Ea30A6_0OriEVREhiq7_FcsMgNIHVrpSxkSqibIIetRYKU1w4uqTCOSBrAzePopz8eTmgADgtEFVtrtQnpNaj1fDR9UrYboFevvmzPUwCWFFynOOP90yVgSZ4qhc",
                "https://lh3.googleusercontent.com/aida-public/AB6AXuDOb8DcRDYaaMW8bLtk7MDJ_OLROdP4poCE6ZX2hWuFf62cqu09MMOBfWTNT2NgG22HMCOP6B0FQ8tfuneWgEXzH5lrWJV1weaCytdNblO-ZAZKRNpQh7fEJM138TJA0wh-sXhMYWqfE_g-dGQOwiR7CtAauJYM4EuC65c5KbEtoKxJl1YIPGO-0yKaGetSeBsYMt0d9_IxKAqNq1IVM5sXr-sTEkdbvs7sSHwUd6UuZzm0Cb8PFLQ8fAdwmXxejIRDCEPVUlf6_Ycs"
              ].map((img, idx) => (
                <div key={idx} className={`animate-on-scroll animate-zoom delay-${idx * 100} aspect-[4/5] overflow-hidden group rounded-sm border border-card-border`}>
                  <img alt={`Style ${idx + 1}`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110 grayscale group-hover:grayscale-0" src={img} />
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 relative border-b border-white/5" style={gridStyle}>
          <div className="container mx-auto px-4 md:px-8 relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
              {[
                { name: 'Carlos Silva', text: 'Melhor barbearia da região. Profissionais qualificados e ambiente incrível.' },
                { name: 'Rafael Santos', text: 'Atendimento impecável e resultado sempre perfeito. Recomendo!' },
                { name: 'Marcos Oliveira', text: 'Agendamento online facilitou muito. Nunca mais perco tempo em fila.' },
                { name: 'Lucas Ferreira', text: 'Qualidade premium por um preço justo. Sou cliente fiel há 2 anos.' }
              ].map((testimonial, idx) => (
                <div key={idx} className={`animate-on-scroll delay-${idx * 100} bg-card-dark border border-card-border p-8 rounded-sm hover:border-orange-500/50 transition-colors`}>
                  <div className="flex gap-1 text-orange-500 text-xs mb-4">
                    <i className="fa-solid fa-star"></i><i className="fa-solid fa-star"></i><i className="fa-solid fa-star"></i><i className="fa-solid fa-star"></i><i className="fa-solid fa-star"></i>
                  </div>
                  <p className="text-gray-400 text-sm italic mb-6">"{testimonial.text}"</p>
                  <p className="text-orange-500 font-bold text-sm uppercase">{testimonial.name}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="py-20 relative overflow-hidden flex justify-center items-center" style={gridStyle}>
          ...
          <div className="relative z-10 w-full max-w-sm px-4 animate-on-scroll animate-zoom">
        ...
        <section className="relative py-24 overflow-hidden flex items-center justify-center bg-[#050505]">
          ...
          <div className="w-full max-w-2xl px-4 relative z-10 animate-on-scroll">
